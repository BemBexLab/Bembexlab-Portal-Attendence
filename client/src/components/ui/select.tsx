...
export function Select({ value, options, onChange, placeholder = "Select", ariaLabel, className, triggerClassName, menuMinWidth = 180 }: {
  value: string;
  ...
  return <div className={cn("relative", className)} data-custom-select>
    <button aria-expanded={open} aria-haspopup="listbox" aria-label={ariaLabel} className={cn("flex h-9 min-w-0 w-full items-center justify-between gap-2 rounded-md border border-input bg-background px-3 text-left text-sm outline-none transition hover:bg-muted/40 focus:ring-2 focus:ring-ring/20", triggerClassName)} onClick={() => setOpen((current) => !current)} ref={triggerRef} type="button">
