...
export async function downloadPayrollXlsx(report: PayrollReport) {
  ...
  sheet.addRow([]);
  sheet.addRow(["EMPLOYEE CODE", "EMPLOYEE", "DEPARTMENT", "MONTHLY SALARY", "PAYROLL DAYS", "ASSESSED WORKING DAYS", "PRESENT", "ABSENT", "HALF DAYS", "HALF-DAY DEDUCTION", "TOTAL DEDUCTION DAYS", "DEDUCTION AMOUNT", "PAYABLE SALARY", "ABSENT DATES", "HALF-DAY DATES"]);

  report.rows.forEach((row) => sheet.addRow([
    row.employeeCode, row.employee, row.department, row.monthlySalary, row.payrollDays,
    row.assessedWorkingDays, row.presentDays, row.absentDays, row.halfDays,
  ...
  [16, 24, 20, 18, 14, 15, 12, 10, 12, 20, 22, 20, 20, 38, 38].forEach((width, index) => { sheet.getColumn(index + 1).width = width; });
  [4, 12, 13].forEach((column) => { sheet.getColumn(column).numFmt = '"PKR" #,##0.00'; });
